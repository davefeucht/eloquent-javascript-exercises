...
Chapter 12: Fixing Scope

Currently, the only way to assign a binding a value is define. This construct acts 
as a way both to define new bindings and to give existing ones a new value.
...
Object.getPrototypeOf function, which returns the prototype of an object. Also remember 
that scopes do not derive from Object.prototype, so if you want to call hasOwnProperty on 
...
specialForms.set = (args, scope) => {
  ...
  if(!Object.prototype.hasOwnProperty.call(scope, args[0].name)) {
    let parent_scope = Object.getPrototypeOf(scope);
    if(!Object.prototype.hasOwnProperty.call(parent_scope, args[0].name)) {
      throw new ReferenceError(args[0].name + " does not exist");
    }
    else {
      parent_scope[args[0].name] = value; 
    }  
  }
  else {
